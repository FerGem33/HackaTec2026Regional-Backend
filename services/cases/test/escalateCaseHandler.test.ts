...
describe("escalateCaseHandler", () => {
  ...
  it("applies ESCALATE, audits it, and never changes AnomalyCases.status (only alertStatus)", async () => {
    ...
    expect(body).toEqual({ caseId: CASE_ID, alertStatus: "ESCALATED" });
    ...
    expect(casesUpdate?.UpdateExpression).toContain("alertStatus = :decision");
