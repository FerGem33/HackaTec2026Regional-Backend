...
export class SenseCareDemoStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Hito 5 (ingesta del simulador web + consulta de solo lectura para
    // simulador/app movil). Cognito emite el JWT (usuarios de demo se
    // crean a mano, ver docs/DEMO_INGEST_AUTH.md); demoIngestHandler
    // valida schema + una allowlist explicita de deviceId (nunca deviceId
    // de una Pi real con certificado X.509) y empuja al MISMO SQS que las
    // IoT Rules -- ver comentario en services/ingestion/src/demoIngestHandler.ts
    // sobre por que nunca produce mqttDeviceId via transporte MQTT real.
    // Las rutas de lectura (getDeviceLatestHandler/getDeviceTelemetryHandler)
    // SI devuelven datos de pi-demo-01 (la Pi fisica real): la allowlist
    // solo restringe quien puede ESCRIBIR por HTTPS, no quien puede leer.
    // El "mecanismo de alta de Devices/recipientId" que un deviceId de demo
    // necesita sigue siendo manual (mismo paso que docs/
    // DEVICE_PROVISIONING_AND_SMOKE_TEST.md Seccion 1, con ese deviceId).
    ...
    // Base para las 3 rutas: POST {url}demo/devices/{deviceId}/events,
    // GET {url}devices/{deviceId}/latest, GET {url}devices/{deviceId}/telemetry
    ...
    // y GET /devices/{deviceId}/telemetry ya estan arriba (DemoIngestApi,
    // solo lectura, sin CaregiverAccess: cualquier JWT valido puede
    // consultar cualquier deviceId). Falta: GET /cases/{caseId}/events y la
    // accion humana autorizada (POST /cases/{caseId}/cancel, /escalate)
    // contra CaregiverAccess -- esas si necesitan verificar que el sujeto
    // del JWT tiene relacion autorizada con el recipientId del caso.
