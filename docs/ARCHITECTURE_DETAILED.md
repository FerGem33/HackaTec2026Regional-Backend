...
| `AnomalyCases` | `caseId` | Estado, `executionArn`, evidencia, plazos, respuestas y resultado de escalamiento. Hito de alertas/escalamiento: agrega `severity?`, y 3 campos deliberadamente separados (nunca un solo `alertStatus` genérico, ver EMERGENCY_CALL_RUNBOOK.md sección 1): `notificationStatus` (espejo liviano de `Alerts`, mismo patrón que `analysisStatus`), `humanDecision` (`CANCELLED`/`ESCALATED`, con `cancelledAt`/`cancelledBy`/`escalatedAt`/`escalatedBy`) y `dialStatus` (`DIALING`/`CALLED`/`BLOCKED`/`FAILED`, con `dialingClaimedAt`/`calledAt`/`connectContactId`/`dialBlockedReason`/`dialFailedReason`); `status` (ciclo de vida de Step Functions) nunca lo escriben las acciones humanas ni el escalamiento. |
| `CaseActionCallbacks` | `caseId` / `callbackType` (implementado, hito de escalamiento) | Tabla efímera (TTL) del `taskToken` de Step Functions mientras la máquina espera una decisión humana (`RequestHumanDecisionFn`/`ReconcileHumanDecisionFn`). El `taskToken` **nunca** se copia a `AnomalyCases`, `EventLog`, `Alerts` ni al estado propio de la ejecución — sólo vive aquí. Ver `services/orchestration/src/caseActionCallbackStore.ts` y EMERGENCY_CALL_RUNBOOK.md. |
...
| `dispatchAlertFn` (implementado) | `sns:Publish` acotado al ARN exacto de `SenseCare-Alerts`; `dynamodb:Query` solo sobre el GSI `CaregiverAccessByDevice`; lectura/escritura de `Alerts`; `UpdateItem` en `AnomalyCases` (espejo de `notificationStatus`). Sin S3, Bedrock ni Connect. |
| `cancelCaseFn` / `escalateCaseFn` (implementado) | `dynamodb:GetItem` en `AnomalyCases`/`CaregiverAccess`, `dynamodb:TransactWriteItems` acotado a `AnomalyCases`+`Alerts`(+`CaseActionCallbacks` si hay un `taskToken` pendiente), `states:SendTaskSuccess` (sin permisos a nivel de recurso, `Resource:"*"` documentado), `PutItem` en `EventLog`. Nunca `sns:Publish` ni `connect:*`: no envían nada ni llaman, solo registran una decisión humana y, si aplica, resuelven el callback de Step Functions en espera. |
| `getCaseEventsFn` (implementado) | `dynamodb:GetItem` en `AnomalyCases`/`CaregiverAccess`, `dynamodb:Query` en `EventLog`. Solo lectura. |
| `requestHumanDecisionFn` (implementado, hito de escalamiento) | `dynamodb:GetItem` en `AnomalyCases`, `dynamodb:PutItem`/`UpdateItem` en `CaseActionCallbacks`, `states:SendTaskSuccess` (`Resource:"*"`, misma excepción documentada que `evidenceCallbackHandler`). Sin Connect/SSM/KMS. |
| `reconcileHumanDecisionFn` (implementado, hito de escalamiento) | Únicamente `dynamodb:UpdateItem` en `CaseActionCallbacks`. Sin Connect/SSM/KMS/States. |
| `escalationPolicyFn` (implementado, hito de escalamiento) | `dynamodb:GetItem`/`UpdateItem` en `AnomalyCases`, `dynamodb:GetItem` en `Devices`, `dynamodb:PutItem` en `EventLog`. Determinista; **sin ningún permiso Connect** (ver AGENTS.md: el LLM/ninguna Lambda salvo `emergencyDialerFn` puede invocar Connect ni cerrar/escalar un caso). |
| `emergencyDialerFn` (implementado, hito de escalamiento) | La ÚNICA Lambda de todo SenseCare con `connect:StartOutboundVoiceContact` (acción sin soporte de permisos a nivel de recurso — única excepción documentada con `Resource:"*"`, revisada y aprobada explícitamente). Además `ssm:GetParameter` acotado al ARN exacto de un solo parámetro y `kms:Decrypt` acotado a su CMK propia. Sin acceso a `AnomalyCases`/`EventLog`. |
| `recordCallOutcomeFn` (implementado, hito de escalamiento) | `dynamodb:UpdateItem` en `AnomalyCases`, `dynamodb:PutItem` en `EventLog`. Sin Connect/SSM/KMS: transiciona `dialStatus` a partir del output ya producido por `emergencyDialerFn`. |
| decisionAgent | Invocar Bedrock y el despachador de herramientas; no puede invocar Connect ni resolver un caso. (Conceptual/roadmap: el hito de escalamiento implementado usa una espera determinista de Step Functions, no un agente Bedrock con herramienta `emergency_call`.) |
